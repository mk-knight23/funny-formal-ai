...
export function SettingsDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      // Load keys (will automatically migrate to encrypted storage)
      getStoredApiKeys()
        .then((keys) => {
          setApiKeys(keys);
        })
  ...
  const handleSave = async () => {
    ...
    try {
      // Use the secure async save function
      ...
      setSaveError("An error occurred while saving API keys.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      ...
      >
        ...
        {/* Tabs */}
        <div
          className={`flex overflow-x-auto border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}
        >
        ...
        >
          <div>
            <p
              className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
            >
              API keys are stored locally with AES-GCM encryption
            </p>
            {saveError && (
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="px-6 py-2 rounded-lg font-semibold bg-gradient-to-r from-fuchsia-500 to-cyan-400 text-white hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed"
            >
